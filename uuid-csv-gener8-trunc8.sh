...
# Run the "uuidgen -hdr" n=3 times and save the results to a CSV
...
# Remove extraneous lines that uuidgen generates by default
for i in warning, define
do
  sed -i '' '/i/d' $pathToCSV".csv"
done

# Truncate the first three non-alphanumeric characters that uuidgen generates by default
